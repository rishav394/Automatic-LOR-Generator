...
function switchPage(page) {
	...
	if (page == 2) {
		...
		if (keepGoing) {
			document.getElementById('javascript-generator').innerHTML =
				...
				"<p>I gather the student's first name to be " +
				firstName +
				'.</p>' +
	...
	if (page == 3) {
		try {
			firstName = document.getElementById('modFirstName').value;
		} catch (e_var_2) {}
	...
	if (page == 4) {
		...
		if (!keepGoing) {
			prevPage();
			console.log('Executed prevpage, no gender option given');
		} else {
			console.log('gender: ' + pronouns[gender] + ' / ' + proref[gender]);
			document.getElementById('javascript-generator').innerHTML =
				'' +
				"<h3>Let's get to know the context of the letter</h3>" +
				'<p>Please specify the type of subject you are recommending ' +
				firstName +
				' to: </p>' +
	...
	if (page == 5) {
		...
		if (!keepGoing) {
			prevPage();
			console.log('Executed prevpage, no programme specified');
		} else {
			if (programType == 0) {
				nextPage();
	...
	if (page == 6) {
		if (programType != 0) {
			keepGoing = true;
			try {
				programName = document.getElementById('instName').value;
			} catch (e_var_0) {}
	...
	if (page == 7) {
		...
		keepGoing = true;
		try {
			otherTrait = document.getElementById('extraTrait').value;
		} catch (e_var_0) {
			override = true;
		}
		console.log('name: ' + studentName);
		...
		if (keepGoing) {
			...
			for (var i = 0; i < traitArr.length; i++) {
				if (traitArr[i]) {
					var traitName = '';
					if (i == 6) {
						traitName = otherTrait;
					} else {
						traitName = traitNames[i];
					}
					result += '<h4>' + traitName + '</h4>';
					result +=
						'Less <input type="range" min="1" max="3" value="2" class="slider" id="traitRange' +
						i +
						'"> More';
				}
			}
	...
	if (page == 9) {
		keepGoing = true;
		try {
			teacherName = document.getElementById('teachName').value;
		} catch (e_var_60) {}
		if (teacherName === '') {
			prevPage();
	...
	if (page == 10) {
		document.getElementById('javascript-generator').innerHTML =
			"<h3>Let's get to know you:</h3>" +
			'<p>In what class did you work with ' +
			studentName +
			'?' +
...
function page6traits(x) {
	if (x == 6 && toggle6) {
		...
		document.getElementById('javascript-generator').innerHTML +=
			'<br><p>Add a trait...<p><input type="text" name="traitbox" placeholder="eg. smart" id="extraTrait">';
		toggle6 = !toggle6;
	} else if (x == 6 && !toggle6) {
